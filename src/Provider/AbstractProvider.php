...
namespace Awx\OAuth2\Client\Provider;

use GuzzleHttp\Client as HttpClient;
use GuzzleHttp\ClientInterface as HttpClientInterface;
use GuzzleHttp\Exception\BadResponseException;
use Awx\OAuth2\Client\Grant\AbstractGrant;
use Awx\OAuth2\Client\Grant\GrantFactory;
use Awx\OAuth2\Client\OptionProvider\OptionProviderInterface;
use Awx\OAuth2\Client\OptionProvider\PostAuthOptionProvider;
use Awx\OAuth2\Client\Provider\Exception\IdentityProviderException;
use Awx\OAuth2\Client\Token\AccessToken;
use Awx\OAuth2\Client\Token\AccessTokenInterface;
use Awx\OAuth2\Client\Tool\ArrayAccessorTrait;
use Awx\OAuth2\Client\Tool\GuardedPropertyTrait;
use Awx\OAuth2\Client\Tool\QueryBuilderTrait;
use Awx\OAuth2\Client\Tool\RequestFactory;
...
abstract class AbstractProvider
{
    ...
    public function getAccessToken($grant, array $options = [])
    {
        ...
        // Special case where refresh token has expired
        if ($response['statusCode'] === 403 && $response['parsed'] ==='The refresh token has expired.') {
            return 'EXPIRED_REFRESH_TOKEN';
        }
        
        if (false === is_array($response['parsed'])) {
            throw new UnexpectedValueException(
                'Invalid response received from Authorization Server. Expected JSON.'
            );
        }
        $prepared = $this->prepareAccessTokenResponse($response['parsed']);
    ...
    public function getParsedResponse(RequestInterface $request)
    {
        ...
        return array('parsed' => $parsed, 'statusCode' => $statusCode);
    }
